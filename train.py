...
if __name__ == '__main__':
    ...
    # 制作数据集
    # [CRFdataset * 4]
    train_datasets = construct_bucket_mean(
        train_features, train_labels, feature2idx, label2idx, Large=args.large)
    dev_datasets = construct_bucket_mean(
    ...
    if checkpoint_file is not None and args.load_opt:
        optimizer.load_state_dict(checkpoint_file['oimptimizer'])
    ...
    # evaluate_by_file(dev_gold, tmp_file, args.log_path, 0)
    ...
    for epoch_idx, args.start_epoch in enumerate(epoch_list):
        ...
        if dev_F1 > best_F1 or dev_P > best_acc:
            # 保存模型
            patience_count = 0
